...
// This above syntax is as similar as below
/*
     struct superblock
	 {
		 int TotalInodes;
...
UFDT UFDTArr[50];              // Creating Array of UFDT Structure
...
void InitialiseSuperBlock()
{
	...
	while(i<MAXINODE)
	{
		UFDTArr[i].ptrfiletable = NULL;  		// Setting all UFDTArr array Elements value to NULL 
		i++;
	}
...
int CreateFile(char *name, int permission)
{
	...
	while(temp != NULL)                     // To find the inode whose FileType is 0 means the inode which is not in use.
	{
		if(temp->FileType == 0)
		{
			break;
	...
	while(i<50)                            // To find unused index of UFDT array to store filetable address
	{
...
int main()
{
	...
	char Command[3][80], str[80], arr[1024];  
	...
	while(1)
	{
		...
		else if(count == 2)
		{
			...
			else if(strcmp(Command[0],"truncate") == 0)
			{
				...
				if(ret == 0)
				{
					printf("Data from the file is deleted\n");
				}
				continue;
			}
			else
